...
for (i in 1:4) {
  for (n in colnames(df)) {
    if (n %in% c("Year", "WkNo", "Cases", "Date")) {
      next
    } else {
...
```{r}
i <- c("avg_temp", "avg_rainfall", "avg_wind", "max_temp", "min_temp", "tot_rainfall", "max_wind")
```
...
```{r}
var_mdl %>% forecast(h=26) %>% autoplot(var_tbl)
```
...
var_mdl <- var_tbl %>%
  model(
    aicc = VAR(vars(Cases,avg_mean_temp), ic="aicc")
  )
...
```{r}
colnames(var_fitted) <- c("Date", "Cases1", "Type")
```
